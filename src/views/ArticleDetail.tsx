...
export default function ArticleDetail({ id, go }: { id: string; go: (v: View) => void }) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-ink-950 p-6 sm:p-8">
          ...
          <div className="relative">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-4">
              ...
              {article.publishedAt && (
                <span className="flex items-center gap-1.5 rounded-full bg-gold-soft px-3 py-1 text-xs font-bold text-gold-deep">
      ...
      {article.excerpt && (
        <Reveal delay={150}>
          <div className="rounded-xl border border-gold/30 bg-gold-soft/30 p-6">
            <Overline>الملخص</Overline>
            <p className="mt-4 text-lg text-ink-700 leading-relaxed italic">
              {article.excerpt}
            </p>
          </div>
        </Reveal>
      ...
      <Reveal delay={200}>
        <div className="rounded-xl border border-line bg-card p-6 sm:p-8">
          ...
          <div className="mt-6 prose prose-lg max-w-none">
            {article.body.split("\n").map((paragraph, index) => (
              <p key={index} className="mb-4 text-ink-700 leading-relaxed">
                {paragraph}
              </p>
            ))}
          </div>
          ...
          <div className="rounded-xl border border-line bg-card p-6">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {article.keywords.map((keyword) => (
                <span
                  key={keyword}
                  className="flex items-center gap-1.5 rounded-full bg-gold-soft px-4 py-2 text-sm font-bold text-gold-deep"
                >
      ...
      {article.tags && article.tags.length > 0 && (
        <Reveal delay={300}>
          <div className="rounded-xl border border-sea/30 bg-sea-soft/30 p-6">
            <Overline>الوسوم (Tags)</Overline>
            <div className="mt-4 flex flex-wrap gap-2" dir="ltr">
              {article.tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full bg-card px-4 py-2 font-mono text-sm font-bold text-sea shadow-sm"
                >
                  #{tag}
                </span>
              ))}
